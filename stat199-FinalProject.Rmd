...
obama <- tweetsWithSentimentWithoutLink %>% filter(obama==1)
ggplot(data=obama, mapping= aes(x=posNeg)) + geom_bar()

biden <- tweetsWithSentimentWithoutLink %>% filter(biden==1)
ggplot(data=biden, mapping= aes(x=posNeg)) + geom_bar()

kamala <- tweetsWithSentimentWithoutLink %>% filter(kamala==1)
ggplot(data=kamala, mapping= aes(x=posNeg)) + geom_bar()

aoc <- tweetsWithSentimentWithoutLink %>% filter(aoc==1)
ggplot(data=aoc, mapping= aes(x=posNeg)) + geom_bar()

pence <- tweetsWithSentimentWithoutLink %>% filter(pence==1)
ggplot(data=pence, mapping= aes(x=posNeg)) + geom_bar()

amy <- tweetsWithSentimentWithoutLink %>% filter(amy==1)
