...
genderDemocrats <- tweetsWithSentimentWithoutLink %>% 
   ...
   filter(obama == 1 | biden ==1 | kamala==1 | pelosi ==1 | aoc ==1 | 
             hillary ==1)
ggplot(data = genderDemocrats, mapping = aes(y = ave_sentiment, x = gender, 
                                             color=gender)) +
   geom_boxplot() +
   labs(title = "Sentiment scores are roughly the same, with more range for 
        male politicians",
...
ggplot(data = genderRepublicans, mapping= aes(y = ave_sentiment, x = gender, 
                                              color=gender)) +
   geom_boxplot() +
