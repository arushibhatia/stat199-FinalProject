...
We created multiple visualizaitons to help us explore our data. In Figure 1, we
created a word cloud of the most common words found in his Tweets. The most 
common words are displayed with the most frequent words being displayed 
larger. Given the results, we can see that some of the words he most commonly 
uses are "great", "Trump", "president", "people", "country", "America", and 
"time." While these words don’t necessarily indicate the full scope of what he 
discusses on his Twitter account, we notice that most of his Tweets are in 
...
words such as “fake”, “false”, “bad”, and “crime”. Given the polarity of his 
Tweets, we believed it would be interesting to see if a common theme would 
develop among his most common words, which is partially indicated in our 
results. 

Similarly, Figure 2 is a frequency chart of his top 20 most used words in his
Tweets. It is interesting to see how much more the word "great" is used in
comparison to the other words displayed on the chart. 
...
making it easier to visualize the results. Overall, it appears that Alexadria 
Ocasio-Cortez, Hillary Clinton, and Nancy Pelosi recieved the highest proportion 
...
proportion compared to the people of the Democratic party. 
...
depending if we were considering a Republican female politician versus a 
Democrat female politician (we just wanted to learn if he had a different 
sentiment towards strictly males or females, or whether it had to also
do with politican affiliation). This visualization shows that when Donald Trump 
...
this question further with a Central Limit Theorem-based t-test to investigate
if there is enough evidence to suggest that he talks, on average, more 
negatively about Democrats than Republicans. Similarly, we will also do another
t-test to investigate if there is enough evidence to suggest that he talks, on
average, more positively about males than females.

Figure 5 allows us to see the change in Donald Trump’s sentiments towards 
...
some specific topics. To investigate this further, we will do a CLT-based
t-test to see if there is enough evidence to sugest that the mean
...
We will do this for a few different topics.
...
#CLIMATE CHANGE
prePostClimateChange <- yearVar %>%
   filter(topic=="climateChange") %>%
   mutate(prePost = case_when(year<2016 ~ "preElection",
                              year>=2016  ~ "postElection"))

chisq.test(table(prePostClimateChange$prePost, 
                 prePostClimateChange$posNeg))
#GUNS 
prePostGuns <- yearVar %>%
   filter(topic=="guns") %>%
   mutate(prePost = case_when(year<2016 ~ "preElection",
...
chisq.test(table(prePostGuns$prePost, 
                 prePostGuns$posNeg))

#IMMIGRATION 
# chi squared test to see if there is an association between whether the Tweet
# was pre election or post election and the average sentiment score

prePostImmigration <- yearVar %>%
   filter(topic=="immigration") %>%
   mutate(prePost = case_when(year<2016 ~ "preElection",
                              year>=2016  ~ "postElection"))

chisq.test(table(prePostImmigration$prePost, 
                 prePostImmigration$posNeg))
...
chisq.test(table(prePostNews$prePost, 
                 prePostNews$posNeg))
...
### Results
Sentiment across political party: In order to determine if the true mean 
...
2.2 * 10$^{-16}$, which is very close to zero and less than our $\alpha$ of 
...
Sentiment across gender: In order to determine if the true mean 
sentiment score of Donald Trump's Tweets about male politicians is greater 
...
know that the distribution of the test statistic can be approximated by a normal 
distribution which means that it is unimodal, symmetric, and centered at zero.
The value of out test statistic, t, is 6.6948. Our p-value is 1.234*10$^{-11}$, 
