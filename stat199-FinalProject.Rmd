...
trumpTweets <- trumpTweets %>%  
   mutate(blm = case_when((grepl("BLM", text , ignore.case = TRUE) | 
                      ...
                      grepl("loot", text , ignore.case = TRUE) |
                        grepl("riot", text , ignore.case = TRUE) |
                        grepl("pigs", text , ignore.case = TRUE)) ~ 1,
                            TRUE ~0))
...
trumpTweets <- trumpTweets %>%  
   mutate(news = case_when((grepl("news", text , ignore.case = TRUE) | 
                              grepl("fake news", text , ignore.case = TRUE) |
                      grepl("CNN", text , ignore.case = TRUE) |
                        grepl("Fox", text , ignore.case = TRUE) |
                        grepl("media", text , ignore.case = TRUE) |
                        grepl("fake media", text , ignore.case = TRUE)) ~ 1,
                            TRUE ~0))
...
```{r}
tweetsWithSentiment <- tweetsWithSentiment %>% mutate(posNeg = 
                           if_else(ave_sentiment>0, "positive", "negative"))
view(tweetsWithSentiment)
obama <- tweetsWithSentiment %>% filter(obama==1)
ggplot(data=obama, mapping= aes(x=posNeg)) + geom_bar()

biden <- tweetsWithSentiment %>% filter(biden==1)
ggplot(data=biden, mapping= aes(x=posNeg)) + geom_bar()

kamala <- tweetsWithSentiment %>% filter(kamala==1)
ggplot(data=kamala, mapping= aes(x=posNeg)) + geom_bar()

aoc <- tweetsWithSentiment %>% filter(aoc==1)
ggplot(data=aoc, mapping= aes(x=posNeg)) + geom_bar()

pence <- tweetsWithSentiment %>% filter(pence==1)
ggplot(data=pence, mapping= aes(x=posNeg)) + geom_bar()

amy <- tweetsWithSentiment %>% filter(amy==1)
ggplot(data=amy, mapping= aes(x=posNeg)) + geom_bar()
```
